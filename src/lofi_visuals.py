...
def _search_bing_nature(query: str) -> list[str]:
    url = f"https://www.bing.com/images/search?q={quote(query + ' nature wildlife 4k')}&form=HDRSC3"
    try:
...
def _generate_pollinations_flux_nature(prompt: str, out_path: Path, seed: int = None) -> bool:
    try:
        clean_p = re.sub(r'[^a-zA-Z0-9\s,.-]', '', prompt).strip()
        enhanced_prompt = f"{clean_p}, wildlife national geographic photography, stunning nature, bright golden hour sunlight, hyper-detailed 8k, photorealistic masterpiece, crisp focus"
...
def _process_image_card(base_img_path: Path, out_path: Path, topic_label: str = "FAKTA ALAM", w: int = 1080, h: int = 1920, is_hook: bool = False):
    try:
        im = Image.open(base_img_path).convert("RGBA")
    except Exception:
        im = Image.new("RGBA", (w, h), (15, 35, 20, 255))
    ...
    im = enh_contrast.enhance(1.14)
    enh_color = ImageEnhance.Color(im)
    im = enh_color.enhance(1.24)
    ...
    if is_hook:
        badge_y = 120
        draw.rectangle([(50, badge_y), (360, badge_y + 60)], fill=(34, 160, 80, 240))
...
def fetch_all(scenes: list[dict], out_dir: Path, words: list[dict] = None, voice_audio: Path = None) -> list[Path]:
    ...
    print(f"    [Nature Visuals] Generating dynamic nature cuts for {len(scenes)} scenes ({total_audio_dur:.1f}s audio)...")
    ...
    for i, scene in enumerate(scenes):
        ...
        found_urls = []
        for q in queries:
            for u in _search_bing_nature(q):
        ...
        for sub_idx in range(num_subclips):
            ...
            if not img_ready:
                prompt = queries[sub_idx % len(queries)] if queries else scene.get("text", "wildlife animal nature phenomenon")
                img_ready = _generate_pollinations_flux_nature(prompt, raw_img_path, seed=clip_counter + 300)
            ...
            if img_ready:
                ...
                _process_image_card(raw_img_path, final_img_path, "FAKTA ALAM", w, h, (i == 0 and sub_idx == 0))
                _image_to_video(final_img_path, out_clip_path, subclip_dur, w, h, fps, zoom_direction=clip_counter)
            else:
                img = Image.new("RGB", (w, h), (20, 50, 25))
        ...
        print(f"    scene {i+1}/{len(scenes)}: {total_scene_dur:.1f}s -> {num_subclips} nature cuts generated")

    print(f"    [Nature Visuals] Ready: {len(all_clips)} dynamic clips covering full video duration.")
    return all_clips
